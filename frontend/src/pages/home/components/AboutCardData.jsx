...
const AboutCardData = ({ data, idx }) => {
  return (
    ...
    >
      <div className="md:p-[3vw] sm:p-[4vw] xs:p-[5vw]">
        <OptimizedImage
          className="md:w-[4.5vw] sm:w-[6.5vw] xs:w-[8.5vw]"
          src={data.icon}
        />
      </div>
      <div className="flex flex-col md:gap-[0.8vw] xs:gap-[1.8vw]">
        <div>
          <h1
            id="aboutName"
            className="md:text-[2.3vw] sm:text-[3.3vw] xs:text-[4.3vw] font-lexend_deca text-themeBlue font-medium transition-all duration-700"
          >
            {data.name}
          </h1>
          <p className="md:text-[1.3vw] sm:text-[2.3vw] xs:text-[3.3vw] text-gray-500 font-jost tracking-wide">
            {data.disc}
          </p>
        </div>
        <div className="flex items-center justify-between">
          <p className="md:text-[1.4vw] sm:text-[2.4vw] xs:text-[3.4vw] text-gray-500">
            {data.projects} Projects
          </p>
          <Link
            id="icon"
            className="md:p-[0.8vw] sm:p-[1.8vw] xs:p-[2.8vw] duration-1000 rounded-full border border-gray-400"
          >
            <ArroIcon className={"md:w-[0.7vw] sm:w-[1.7vw] xs:w-[2.7vw]"} />
          </Link>
        </div>
      </div>
    </motion.div>
